...
const serializeGlobalConfig = (rows: Array<any>): Array<ABBGlobalConf> => {
	const serialized = rows.map(row => {
		const rowCopy = {
			'email': '',
			'password': '',
...
const getSiteConfigBySiteId = async (siteId: string): Promise<ABBSiteConf> => {
...
const getGlobalConfByOrganization = async (organizationId: string): Promise<Array<ABBGlobalConf>> => {
	const qs = `
...
const getGlobalConfByEmail = async  (email:string): Promise<ABBGlobalConf> => {
	const qs = `
...
const getAllSiteConfigurations = async (): Promise<Array<ABBSiteConf>> => {
	const res = await dbClient.query(
